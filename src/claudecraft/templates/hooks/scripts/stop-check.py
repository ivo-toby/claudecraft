#!/usr/bin/env python3
"""
ClaudeCraft Stop Hook - Runs when Claude Code finishes responding.
...
Environment variables:
  - CLAUDECRAFT_STOP_REQUIRE_COMMIT: If "true", block if there are uncommitted changes
  - CLAUDECRAFT_STOP_REQUIRE_TESTS: If "true", block if tests weren't mentioned
  - CLAUDECRAFT_STOP_GENERATE_DOCS: If "true", trigger documentation generation
  - CLAUDECRAFT_PROJECT_ROOT: Root of the ClaudeCraft project
"""
...
def trigger_docs_generation(project_root: str, transcript_path: str) -> None:
    ...
    if spec_id:
        ...
        print(f"[ClaudeCraft] Triggering documentation generation for spec: {spec_id}", file=sys.stderr)

        try:
            # Run docs generation asynchronously (don't block)
            subprocess.Popen(
                ["claudecraft", "generate-docs", "--spec", spec_id],
                cwd=project_root,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
        except Exception as e:
            print(f"[ClaudeCraft] Warning: Could not trigger docs generation: {e}", file=sys.stderr)
...
def main():
    ...
    project_root = os.environ.get("CLAUDECRAFT_PROJECT_ROOT", os.getcwd())
    require_commit = os.environ.get("CLAUDECRAFT_STOP_REQUIRE_COMMIT", "false").lower() == "true"
    require_tests = os.environ.get("CLAUDECRAFT_STOP_REQUIRE_TESTS", "false").lower() == "true"
    generate_docs = os.environ.get("CLAUDECRAFT_STOP_GENERATE_DOCS", "false").lower() == "true"
