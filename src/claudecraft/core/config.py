"""Configuration management for ClaudeCraft."""
...
def find_project_root(start: Path | None = None) -> Path | None:
    """Find the project root by looking for .claudecraft directory."""
    current = start or Path.cwd()
    while current != current.parent:
        if (current / ".claudecraft").is_dir():
            return current
...
@dataclass
class Config:
    """ClaudeCraft project configuration."""
    ...
    def load(cls, path: Path | None = None) -> "Config":
        ...
        if path is None:
            ...
            if project_root is None:
                raise FileNotFoundError("No .claudecraft directory found")
            path = project_root / ".claudecraft" / "config.yaml"
        else:
