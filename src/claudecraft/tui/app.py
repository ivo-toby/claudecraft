"""Main TUI application for ClaudeCraft."""
...
from claudecraft.tui.widgets.config_screen import ConfigScreen
from claudecraft.tui.widgets.dependency_graph import DependencyGraph
from claudecraft.tui.widgets.help_screen import HelpScreen
from claudecraft.tui.widgets.new_spec_screen import NewSpecScreen
from claudecraft.tui.widgets.spec_editor import SpecEditor
from claudecraft.tui.widgets.specs import SpecSelected, SpecsPanel
from claudecraft.tui.widgets.swimlanes import SwimlaneScreen


class ClaudeCraftApp(App):
    """ClaudeCraft TUI application."""
    ...
    TITLE = "ClaudeCraft - Spec-Driven Development Orchestrator"
    SUB_TITLE = "TUI Mode"

    def __init__(self, project_path: Path | None = None):
        """Initialize ClaudeCraft app."""
        super().__init__()
    ...
    def on_mount(self) -> None:
        ...
        if self.project_path:
            try:
                self.project = Project.load(self.project_path / ".claudecraft" / "config.yaml")
                ...
                self.sub_title = "No project loaded - use /claudecraft.init"
                return
...
def run_tui(project_path: Path | None = None) -> None:
    """Run the ClaudeCraft TUI application."""
    app = ClaudeCraftApp(project_path)
    app.run()
